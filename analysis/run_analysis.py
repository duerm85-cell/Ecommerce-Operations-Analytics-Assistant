...
def metrics(t: dict[str, pd.DataFrame]) -> dict[str, float]:
    ...
    return {
        ...
        "gross_margin": float((completed.net_sales - completed.cost).sum() / completed.net_sales.sum()),
        "aov": float(completed.net_sales.sum() / completed.order_id.nunique()), "users": int(completed.user_id.nunique()),
        "repeat_rate": float(user_orders.ge(2).mean()), "ctr": float(a.clicks.sum() / a.impressions.sum()),
        "cvr": float(a.conversions.sum() / a.clicks.sum()), "cpc": float(a.spend.sum() / a.clicks.sum()),
        "cpa": float(a.spend.sum() / a.conversions.sum()), "roas": float(a.attributed_revenue.sum() / a.spend.sum()),
    }
...
def analyze(t: dict[str, pd.DataFrame]) -> dict[str, pd.DataFrame]:
    ...
    ads["ctr"]=ads.clicks/ads.impressions; ads["cvr"]=ads.conversions/ads.clicks; ads["cpc"]=ads.spend/ads.clicks; ads["cpa"]=ads.spend/ads.conversions; ads["roas"]=ads.revenue/ads.spend
...
def dashboard_image(path: Path, title: str, subtitle: str, cards: list[tuple[str,str]], labels: list[str], values: list[float], insight: str, accent: str=BLUE) -> None:
    ...
    for i,(label,value) in enumerate(cards):
        x=78+i*(card_w+18); d.rounded_rectangle((x,150,x+card_w,260),radius=14,fill="#F8FAFC",outline="#E7ECF3")
        d.text((x+20,170),label,font=_font(15),fill=MUTED); d.text((x+20,207),value,font=_font(25,True),fill=INK)
    d.text((78,305),"Key comparison",font=_font(19,True),fill=INK)
    maxv=max(values) if values else 1
    for i,(lab,val) in enumerate(zip(labels[:6],values[:6])):
        y=355+i*50; d.text((78,y),lab[:22],font=_font(14),fill=INK)
        d.rounded_rectangle((285,y,285+700*val/maxv,y+24),radius=7,fill=accent)
        d.text((1000,y),f"{val:,.1f}",font=_font(14,True),fill=INK)
...
def export(t: dict[str,pd.DataFrame], a: dict[str,pd.DataFrame], m: dict[str,float]) -> None:
    ...
    for name,frame in a.items(): frame.to_csv(REPORTS/f"{name}_analysis.csv",index=False,encoding="utf-8-sig")
    ...
    dashboard_image(REPORTS/"dashboard_overview.png","01 · Operations overview","Are growth and profit healthy?",[("GMV",f"NT$ {m['gmv']/1e6:.1f}M"),("Net sales",f"NT$ {m['net_sales']/1e6:.1f}M"),("Gross margin",f"{m['gross_margin']:.1%}"),("AOV",f"NT$ {m['aov']:,.0f}"),("Repeat rate",f"{m['repeat_rate']:.1%}")],monthly.month.tolist(),monthly.net_sales.div(1e6).tolist(),"Protect promotion-driven volume with margin floors; track net sales, profit and AOV together.")
