...
2. In Power BI Desktop choose **Get data → Text/CSV** and import products, users, orders, ads and calendar. The pipeline enriches the Power BI `users.csv` with Recency, Frequency, Monetary, three scores and `rfm_segment` after order analysis.
...
- **01 Operations overview** — KPI cards: GMV, Net Sales, Completed Orders, Gross Profit, AOV, Repeat Rate FY2025. Monthly combo chart for Net Sales and Gross Margin. Category contribution bar chart. Slicers: date, category, region, acquisition channel. The FY2025 repeat card intentionally ignores date slicers so every view keeps an equal observation window. Insight box: growth must be interpreted with profit.
