"""Generate reproducible, business-shaped synthetic e-commerce data.

All rows produced by this module are synthetic. No Shopee user, shop, or seller
data is used. Currency is TWD and dates are interpreted in Asia/Taipei.
"""
...
def generate(output: Path, samples: Path, powerbi: Path, products_n: int, users_n: int, orders_n: int) -> dict[str, int]:
    rng = np.random.default_rng(SEED)
    tables = {
        "products": make_products(rng, products_n),
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    print(json.dumps(generate(args.output, args.samples, args.powerbi, args.products, args.users, args.orders), indent=2))
